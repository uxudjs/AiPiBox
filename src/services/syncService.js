...
class SyncService {
  ...
  /**
   * 构造请求认证头
   * 使用 syncId 自身作为 HMAC 密钥，对 "syncId:timestamp" 签名。
   * 只有能够派生出正确 syncId 的客户端（即持有正确密码的用户）才能通过服务端校验。
   * @private
   * @param {string} syncId - 由密码 PBKDF2 派生的同步标识符
