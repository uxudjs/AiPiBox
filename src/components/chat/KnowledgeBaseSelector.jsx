...
import { X, Check, BookOpen, FileText, ChevronRight, Search } from 'lucide-react';
...
const KnowledgeBaseSelector = ({ onClose, onSelectKnowledgeBase, inline = false }) => {
  ...
  return inline ? (
    ...
    <div className="bg-card rounded-3xl shadow-2xl max-w-full w-full max-h-[500px] flex flex-col border" onClick={(e) => e.stopPropagation()}>
      {/* 头部 */}
      ...
      <div className="flex gap-2 p-4 border-t bg-accent/10">
        ...
        <button
          onClick={handleOpenSettings}
          className="flex-1 py-2 bg-accent rounded-xl hover:bg-accent/80 transition-colors text-sm font-medium flex items-center justify-center gap-1.5"
        >
        ...
        <button
          ...
          disabled={!selectedKB}
          className={cn(
            "flex-1 py-2 rounded-xl transition-colors text-sm font-medium",
            selectedKB
